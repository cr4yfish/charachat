...
export default async function OwnCharsPage({ searchParams }: { searchParams: Promise<{ page?: string | undefined }>}) {
    ...
    const calculatedCursor = (requestedPage - 1) * LIMITS.MAX_PERSONAS_PER_PAGE; // Calculate cursor based on page number
    ...
    const chars = await getOwnPersonas({
        cursor:  calculatedCursor,
        limit: LIMITS.MAX_PERSONAS_PER_PAGE // Adjust the limit as needed
    });
