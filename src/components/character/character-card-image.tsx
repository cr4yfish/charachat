                ...
                <Card className="h-[200px] w-[150px] relative py-3 border shadow-none transition-all hover:transform-3d" >
                    ...
                    <CardContent className="absolute z-10 bottom-0 left-0 w-full p-0 overflow-visible rounded-b-2xl">
                        <div className="flex flex-col pb-2 px-3 pt-1 z-10 relative">
                            <h3 className="font-medium text-sm truncate ">{truncateText(props.data.name, 15)}</h3>
                            {props.data.description && <p className="text-xs truncate text-muted-foreground opacity-75 w-[80%] " >{props.data.description}</p>}
                        </div>
                        {/* Background gradient */}
                        <div className="absolute bottom-0 left-0 size-full h-[150%] bg-gradient-to-t from-neutral-900/75 via-neutral-900/33 to-neutral-900/0"></div>
                    </CardContent>
