...
function PurePersonaSmallCard(props: Props) {
    ...
    return (
        ...
        <Link href={`/p/${props.data.id}`} className="w-full max-w-[560px]" onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
            ...
            >
                <Card className={cn("h-[72px] sm:h-[90px]  w-full overflow-hidden p-3 dark:bg-slate-800/5 border shadow-none dark:hover:bg-slate-800/30 flex flex-row items-center justify-start gap-3 transition-all", {
